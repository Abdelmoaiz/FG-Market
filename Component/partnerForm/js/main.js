...
setTimeout(() => {
    ...
    ipcRenderer.on('get-invoiceSup',(e,args)=>{
        ...
        for(let i=0; i<suppliers[suppliers.length-1].length; i++){
            ...
            for(let x=0; x<suppliers[suppliers.length-1][i].newInvoiceBuy.length; x++){
                totalFarkPrice = +data[i].newInvoiceBuy[x].totalPrice - +data[i].newInvoiceBuy[x].total;
                totalNowlon = +data[i].newInvoiceBuy[x].nowlone;
                totalCommission = (+data[i].newInvoiceBuy[x].total - totalNowlon) * +data[i].newInvoiceBuy[x].commission;
                // totalGain = (+totalCommission + +totalFarkPrice);
                totalGain += 50;
                totalPaid = totalPaid;
    ...
    ipcRenderer.on('get-Partner',(e,args)=>{
        ...
        for(let i=0; i<partner[partner.length-1].length; i++){
            ...
            for(let x=0; x<data[i].myPartner.length;x++){
                totalCapital += +data[i].myPartner[x]. paidPrice;

            }

            // for(let x=0; x<data.myPartner.length; x++){

            // }
            

            // if(data[i].myPartner[0].idPartner == i+1){
                // console.log(data)
