...
createSuppliers.addEventListener('click',e =>{
    ...
    if(nameSuppliers.value != '') {
        ...
        if(mood == 'create'){
            customer.push(newSuppliers)
            ipcRenderer.send('myCreditorCustomer',customer)
            window.location.reload();


        }else if(mood == 'update'){
            ipcRenderer.send('update-creditorCustomer',{...newSuppliers, tmpId})
            window.location.reload();

        }else if(mood == 'add'){
            suppliers[suppliers.length-1][tmp].myCreditorCustomer.push(newSuppliers);
            let data = suppliers[suppliers.length-1][tmp];
            ipcRenderer.send('update-creditorCustomer',{...data, tmpId})
            window.location.reload();
...
function addPrice(id){
    ...
    for(let i=0; i<suppliers[suppliers.length-1].length;i++){
        for(let x=0; x<suppliers[suppliers.length-1][i].myCreditorCustomer.length;x++){

            if(suppliers[suppliers.length-1][i]._id == id){
                let data = suppliers[suppliers.length-1][i].myCreditorCustomer;
...
function checkPass(){
    ipcRenderer.send('delete-creditorCustomer',tmpIdDelete)
    window.location.reload();
}
...
setTimeout(() => {
    tableSuppliers = '';
    ipcRenderer.send('get-creditorCustomer', 'bing')
    ipcRenderer.on('get-creditorCustomer',(e,args)=>{
        ...
        for(let i=0; i<suppliers[suppliers.length-1].length; i++){
            ...
            tableSuppliers += `
            <tr>
                <td>${data[i].myCreditorCustomer[0].date}</td>
                <td>${data[i].myCreditorCustomer[0].user}</td>
                <td>${data[i].myCreditorCustomer[0].nameSuppliers}</td>
                <td>${data[i].myCreditorCustomer[0].idPersonalSuppliers}</td>
                <td>${data[i].myCreditorCustomer[0].phoneSuppliers}</td>
                <td>${data[i].myCreditorCustomer[0].adress}</td>
    ...
    for(let i=0; i<suppliers[suppliers.length-1].length;i++){
        for(let x=0; x<suppliers[suppliers.length-1][i].myCreditorCustomer.length;x++){
            ...
            if(id == suppliers[suppliers.length-1][i]._id){
                let data = suppliers[suppliers.length-1];
                totalPriceSuppliers += +data[i].myCreditorCustomer[x].price;
                totalPaidSuppliers += +data[i].myCreditorCustomer[x].paidPrice;
                totalRemainSuppliers = +totalPriceSuppliers - +totalPaidSuppliers;
                document.querySelector('.headTable').innerHTML = `${data[i].myCreditorCustomer[0].nameSuppliers}`

                tableDetailsSuppliers += `
                <tr>
                    <td>${data[i].myCreditorCustomer[x].date}</td>
                    <td>${data[i].myCreditorCustomer[0].nameSuppliers}</td>
                    <td>${data[i].myCreditorCustomer[x].title}</td>
                    <td>${+data[i].myCreditorCustomer[x].price}</td>
                    <td>${data[i].myCreditorCustomer[x].paidPrice}</td>
                    <td>${+data[i].myCreditorCustomer[x].price - +data[i].myCreditorCustomer[x].paidPrice}</td>
...
function searchSuppliers(value){
    ...
    if(value != ''){
        ...
        for(let i=0; i<suppliers[suppliers.length-1].length; i++){
            let data = suppliers[suppliers.length-1];
            if(data[i].myCreditorCustomer[0].nameSuppliers.includes(value)){
                tableSuppliers += `
                <tr>
                    <td>${data[i].myCreditorCustomer[0].date}</td>
                    <td>${data[i].myCreditorCustomer[0].user}</td>
