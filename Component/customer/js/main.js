...
setTimeout(() => {
    ...
    ipcRenderer.on('get-Customer',(e,args)=>{
        ...
        if(suppliers[suppliers.length-1].length != 0 && suppliers !== '') {
            for(let i = 0; i < suppliers[suppliers.length-1].length; i++) {
                if(suppliers[suppliers.length-1][i].mySuppliers != ''){
...
createSuppliers.addEventListener('click',e =>{
    ...
    if(nameSuppliers.value != '') {
        ...
        if(mood == 'create'){
            ...
            ipcRenderer.send('myCustomer',newCustomer,`${idCustomer}`)
            window.location.reload();


        }else if(mood == 'update'){
            ipcRenderer.send('update-customer',{...newCustomer, tmp})
            window.location.reload();
